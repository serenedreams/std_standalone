...
use frame_support::{Parameter, decl_module, decl_event, decl_storage, decl_error, ensure, dispatch, traits::{Get}};
use sp_runtime::traits::{AtLeast32Bit, Zero, Member};
use frame_system::ensure_signed;
use sp_core::U256;
use sp_runtime::{FixedU128, ModuleId, traits::{UniqueSaturatedInto, UniqueSaturatedFrom, MaybeSerializeDeserialize}};
        ...
        pub fn mint_liquidity(origin, token0: AssetId, amount0: Balance, token1: AssetId, amount1: Balance) -> dispatch::DispatchResult {
            ...
            match Pairs::get((token0.clone(), token1.clone())) {
                ...
                None => {
                    ...
                    // Issue LPtoken
                    let lptoken_id: AssetId = <pallet_asset_registry::Module<T>>::get_or_create_asset((*b"lptoken").to_vec())?.into();
